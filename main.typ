...
Данная выпускная квалификационная работа посвящена решению актуальной задачи компенсации траекторных отклонений при синтезе радиолокационных изображений, полученных с помощью радиолокаторов с синтезированием апертуры (РСА) авиационного базирования. Главной проблемой получения качественных изображений поверхности с использованием РСА авиационного базирования является нестабильность траектории при съемке. Современные системы навигации (ГНСС и инерциальные датчики) позволяют определять положение и ориентацию носителя и использовать эти данные для компенсации ТН без использования вычислительно-затратной процедуры автофокусировки. Целью работы является реализация такой компенсации для дальностно-доплеровского алгоритма и её верификация на модельных и реальных данных.
...
В результате выполнения работы по результатам оценки параметров функции отклика точечных целей после применения процедур компенсации было достигнуто восстановление требуемого качество РЛИ. Подробно рассмотрены различные процедуры компенсации ТН, сделан вывод о результатах компенсации и о пределах их применимости. Также в работе представлена оценка качества полученных изображений, выполняемая на основе анализа функции отклика точечных целей по следующим параметрам: разрешение по азимуту и дальности, относительный и интегральный уровень мощности боковых лепестков.
...
#heading(level: 1, numbering: none, [Содержание], outlined: false)
...
#heading(level: 1, numbering: none, [Обозначения и сокращения])
...
* Объект исследования *: радиолокационные и навигационные данные

* Предмет исследования *: алгоритмы компенсации траекторных нестабильностей 
...
Циклограмма сигнала, излучаемого и принимаемого антенной системой РСА, изображена на рисунке @fig:recv_signal. В начале периода излучается сигнал длительностью $t_r$. Затем аппаратура переключается на приём. Принятый сигнал записывается с момента $t_1$ (соответствующего началу прихода отражённого сигнала от объектов на ближней границе $R_1$), до момента $t_2$ (соответствующего окончанию прихода отражённого сигнала от объектов на дальней границе $R_2$). Описанный цикл повторяется с интервалом _PRI_.
...
Дальностно-доплеровский алгоритм разработан в 1978 году и является одним из наиболее распространённых алгоритмов синтеза РЛИ @cumming_sar. Блок-схема алгоритма с модификациями для коррекции ТН (будут рассмотрены далее) представлена на рисунке @fig:RDA_model. Отличительной особенностью алгоритма является операция коррекции миграции дальности, которая выполняется во временной области по дальности и в частотной области по азимуту (области доплеровских частот), т.е. в дальностно-доплеровской области. Математические преобразования, осуществляемые на каждом шаге, и их смысл рассмотрены далее.
...
Структурная схема алгоритма представлена на рисунке @fig:Map-Drift_model. Исходную радиоголограмму делят на две неперекрывающиеся подапертуры, по каждой из которых производят синтез РЛИ с использованием опорной функции, рассчитанной на основе баллистических параметров. После детектирования каждого из амплитудных РЛИ --- вычисляют их корреляционную функцию. Для устранения ошибок более высокого порядка, чем второй, голограмму делят на несколько пар парциальных апертур с совместной обработкой с совместной обработкой каждой пары амплитудных РЛИ для расчёта коэффициентов разложения в ряд.
...
В результате влияния траекторных нестабильностей изменяется дальность до цели (рис. @fig:MOCO_geometry), тем самым изменяется фаза отклика цели и его момент регистрации (рис. @fig:recv_signal_error), то есть отклику данной цели будет соответствовать иное положение на оси времени и, соответственно, дальности. В данном методе изменение фазы компенсируется умножением на согласованный фильтр, зависящим от времени по азимуту и от искажения траекторного сигнала на заданной дальности
...
Таким образом, в данной главе рассмотрены основные принципы радиолокации с синтезированием апертуры, схема работы РСА, процесс получения радиолокационных изображений и приведён анализ алгоритмов синтеза РЛИ. Также был представлен анализ существующих подходов к компенсации ТН и обоснование выбора методов компенсации.
...
Оценка качества полученных изображений производится по следующим параметрам: разрешение по азимуту и дальности, относительный и интегральный уровень мощности боковых лепестков. Алгоритм анализа отклика цели представлен на рисунке @fig:image_analysis_model.
  ...
  На полученном изображении определяется положение максимума и производится сечение профиля через найденный максимум по осям азимута и дальности. Полученное сечение позволяет оценить выбранные параметры: разрешение определяется по ширине главного лепестка на высоте -3 дБ, относительный уровень боковых лепестков определяется как отношение максимума мощности первого бокового лепестка к максимуму мощности центрального лепестка, интегральный уровень боковых лепестков определяется как отношение площади под всеми боковыми лепестками к площади под центральным лепестком по амплитуде. Вывод о результатах компенсации траекторный нестабильностей двумя методами и о пределах их применимости делается по итогам анализа параметров функций отклика всех точечных целей, заданных на сцене. Пример последовательности работы с РЛИ для определения данных параметров изображена на рисунке @fig:image_analysis.
...
Отдельно также необходимо ввести критерий сфокусированности изображения. Поскольку расфокусировка изображения возникает вследствие искажений фазы, в данном случае нельзя применить амплитудный критерий Рэлея для определения предельных значений параметров функции отклика, так отклики целей могут складываться в разной фазе. Поэтому такие критерии определяются исходя из требований максимальной фазовой ошибки отклика цели относительно средней фазы изображения, и, вообще говоря, зависят от системы. Например, для космической системы Кондор-ФКА эта величина составляет 15#sym.degree @kondor. Исходя из этих соображений, в литературе @cumming_sar, @chen_sar сформулированы такие критерии сфокусированности РЛИ:

- Ухудшение разрешения не более, чем в 1,5 раза от теоретического;
- Относительный уровень боковых лепестков не менее -9 дБ (при теоретическом значении в -13 дБ для функции ~sinc без применения сглаживающих окон)
- Интегральный уровень боковых лепестков не менее -7 дБ (при теоретическом значении в -10 дБ в аналогичном случае)

Таким образом, в данной главе описана модель сигнала РСА авиационного базирования и её реализация. Отдельно описана процедура оценки качества изображения на основании параметров функции отклика цели. Также введён критерий сфокусированности изображения.
...
#let threshold_res = 1.5
#let threshold_pslr = -9
#let threshold_islr = -7
...
Также на рисунке @fig:no_dis_scene -- @fig:no_dis_targets представлено изображение сцены и РЛИ всех целей.
...
Рассмотрим эффекты, возникающие на промежуточных этапах обработки голограммы и на конечном изображений вследствие наличия ТН. Значения параметров модели приведены в таблице @tab:no_corr_model_params. Координаты целей аналогичны предыдущему пункту @ref:no_dis (см. таб.у @tab:target_coords). Значение амплитуды искажения траектории выбрано из начала диапазона, чтобы продемонстрировать начало разрушения изображения с не слишком большими искажениями.
...
Приведём также полученные изображения всех целей (рис. @fig:no_corr_all_targets). Отчётливо видно, что даже искажение менее величины разрешения по дальности уже приводит к существенной расфокусировки РЛИ. Определение параметров функций отклика в данном случае не имеет смысла. 
...
Теперь проведём симуляции во для всех значений параметров ТН и построим соответствующие зависимости измеренных параметров функций отклика целей от амплитуды и частоты ТН, которые изображены на рисунке @fig:prec_corr_res_graph, @fig:prec_corr_pslr_graph, @fig:prec_corr_islr_graph. Каждая точка на графике отображает усреднённое по всем целям разрешение. Как и было предсказано, на графики видно экспоненциальное ухудшение разрешения при увеличении амплитуды ТН.
...
#let approx_corr_res = csv("images/approx_sim/sim2_T1/results.csv")
 ...
 Теперь проведём симуляции во для всех значений параметров ТН и построим соответствующие зависимости измеренных параметров функций отклика целей от амплитуды и частоты ТН, которые изображены на рисунке @fig:approx_corr_res_graph, @fig:approx_corr_pslr_graph, @fig:approx_corr_islr_graph. Каждая точка на графике отображает усреднённое по всем целям разрешение. Как и было предсказано, на графики видно экспоненциальное ухудшение разрешения при увеличении амплитуды ТН.
...
Таким образом, была подтверждена корректность реализации дальностно-доплеровского алгоритма синтеза РЛИ. Затем было продемонстрировано влияние ТН на получаемое изображение, заключающееся во внесении ошибок в фазовые характеристики отклика цели; рассмотрено влияние этих ошибок на разных этапах синтеза. Исходя из величины допустимой фазовой ошибки отклика цели введён критерий расфокусированности РЛИ. На основании данного критерия сделан вывод о пределах применимости процедур компенсации. «Приближённый» метод справляется с компенсации хуже «базового», т.е. расфокусировка скомпенсированного данным методом РЛИ наступает при меньших величинах амплитуды ТН. Однако, в силу меньшей вычислительной сложности, «приближённый» метод может применятся для быстрой оценки качества синтеза и компенсации ТН.
...
Аналогично предыдущим пунктам, рассмотрим соответствующие этапы синтеза РЛИ. На рисунке @fig:no_corr_dR_big приведены графики искажения наклонных дальностей, результат которых можно наблюдать на этапах синтеза из рисунка @fig:no_corr_fragments_big и полученных РЛИ целей на рисунке @fig:no_corr_all_targets_big в виде полной расфокусировки изображений. На фрагменте рисунке @fig:no_corr_phase_rcmc_big для цели #2 обратим внимание, что её отклик уже не близок к прямой линии, а оказывается существенно искривлён.
...
Применим "базовый" метод для компенсации искажений. В соответствии с процедурой алгоритма, у всех целей практически отсутствует фазовый набег (рис. @fig:PrCorr_phase_rcmc_big). Фазовые искажения всех целей в целом исправлены, и цели изображения целей будут сфокусированы (рис. @fig:PrCorr_all_targets_big). Отметим также, что благодаря компенсации ошибки времени регистрации отклика цели, амплитуда откликов (рис. @fig:PrCorr_target_rcmc_big) практически полностью выпрямляется, что вносит вклад в компенсацию отклонений.
...
В отличие от предыдущего пункта, "приближённому" методу не удаётся скомпенсировать ТН. Существенный статочный набег фазы (рис. @fig:ApCorr_phase_rcmc_big) и искривление амплитуды отклика (рис. @fig:ApCorr_target_rcmc_big), вызванное существенно большей ошибкой времени регистрации приводят к тому, что РЛИ всех целей оказываются расфокусированы, что показано на рисунке @fig:ApCorr_all_targets_big.
...
#figure(
 ...
 Таким образом, с использованием «приближённого» метода не удаётся скомпенсировать ТН, изображения всех целей оказываются полностью расфокусированы, в то время как  «базовый» метод позволяет сфокусировать РЛИ до допустимого качества благодаря операции исправления времени регистрации отклика цели, как и было предсказано в теоретическом описании данных процедур.  
...
В теоретической части работы рассмотрены основные принципы радиолокации с синтезированием апертуры, схема работы РСА, процесс получения радиолокационных изображений и приведён анализ алгоритмов синтеза РЛИ. Для синтеза изображений выбран дальностно-доплеровский алгоритм как наиболее простой в реализации, а также позволяющий проводить контроль промежуточных этапов синтеза на каждом шаге. Также в работе представлен анализ существующих подходов к компенсации ТН. Данные подходы можно разделить на два класса: алгоритмы автофокусировки и алгоритмы, основанные на применении фазовых множителей. Первый подход заключается в том, что для высокоточного измерения фазовых искажений используется информация о фазовых нестабильностях, содержащейся в самой голограмме или РЛИ. Альтернативой являются методы, использующие навигационные данные, полученные от БИНС носителя для вычисления искажения траектории, и затем для их компенсации. С развитием навигационных систем появилась возможность с приемлемой точностью определять положение и ориентацию носителя РСА во время съёмки, что делает данное направление перспективным для изучения. Целью работы является реализация такой компенсации для дальностно-доплеровского алгоритма и её верификация на модельных и реальных данных.

В работе для оценки качества работы алгоритмов компенсации в среде Matlab была разработана модель сигнала РСА авиационного базирования, позволяющая задавать геометрию сцены и траекторию носителя. Моделируемыми объектами являются точечные отражатели. Искажение траектории представляется в виде гармонической функции, прибавляемой к одной из осей координат носителя. Компенсация траекторных отклонений осуществляется двумя методами, основанными на исправлении фазы откликов цели: «базовым» и «приближённым». Оценка качества полученных изображений выполняется на основе анализа функции отклика точечных целей по следующим параметрам: разрешение по азимуту и дальности, относительный и интегральный уровень мощности боковых лепестков. 

Для моделирование работы алгоритмов компенсации симуляции производится в следующем порядке: сначала проверяется реализации ДДА; затем демонстрируется влияние траекторных искажений на изображение; после чего проводится исследование алгоритмов компенсации при различных параметрах ТН. Рассматривается компенсация на масштабе ТН порядка размера ячейки изображения, и на масштабе, существенно большем данного размера. На каждом этапе проводится анализ промежуточных и итоговых результатов работы модели для оценки соответствующих параметров. При исследовании алгоритмов компенсации определяется изменение измеренного разрешения по азимуту (как основной параметр функции отклика) при увеличении амплитуды траекторных отклонений. Данная зависимость исследуется при изменении других параметров ТН, таких как период функции отклонения. На основании данных измерений делается вывод о допустимой величине ТН для каждого алгоритма и пределах применимости данных алгоритмов.

В результате выполненных исследований поставленная цель достигнута, по результатам оценки параметров функции отклика точечных целей после применения процедур компенсации было восстановлено требуемое качество РЛИ. Исходя из величины допустимой фазовой ошибки отклика цели введён критерий расфокусированности РЛИ. На основании данного критерия сделан вывод о пределах применимости процедур компенсации. «Приближённый» метод справляется с компенсации хуже «базового», т.е. расфокусировка скомпенсированного данным методом РЛИ наступает при меньших величинах амплитуды ТН. Также, «базовый» метод позволяет исправлять искажения траектории, существенно большие размера ячейки изображения. Однако, в силу меньшей вычислительной сложности, «приближённый» метод может применяться для быстрой оценки качества синтеза и компенсации ТН на низкопроизводительных системах.
