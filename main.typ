...
#figure(
  table(
    ...
    [_prf_], [720 Гц],
    [$f_0$], [9,275 ГГц],
    [$y_0$], [$-1000 dot tan(a l p h a slash 2) $ м],
    [$R_"min"$], [2000 м],
    [$R_"max"$], [3800 м],
    [$T$], [3,5 с],
    [*_dis_x_ampl_*], [*0 м*],
...
Рассмотрим подробнее голограмму после коррекции миграции дальности. На @fig:NoCorr_phase_rcmc можно заметить, что набег фазы соответствует искажению траектории, показанном на @fig:NoCorr_dR. Также стоит отметить, что операция коррекция миграции дальности позволяет исправить искривления отклика цели вдоль оси азимута (в сравнении с искажениями на @fig:NoCorr_dR), но не набег фазы, который и вносит основной вклад в размытия РЛИ. Данный набег и предстоит убрать для исправления размытия РЛИ, т.е. должен остаться только линейный набег фазы. Далее в работе вид графика фазы на данном шаге будет использоваться для оценки качества работы процедур компенсации.
...
Приведём также полученные изображения всех целей (см. @fig:NoCorr_all_targets). Отчётливо видно, что искажение порядка половины размера ячейки изображения по дальности приводит к существенной расфокусировки РЛИ.
...
=== Симуляция 2
Проведём анализ аналогично предыдущему подпункту.
...
Значения параметров модели, координат целей и амплитуд искажений аналогичны предыдущем пунктам и  приведены в /*@tab:*/. Теперь добавим процедуру компенсации ТН к модели и проведём аналогичные симуляции.
...
Отдельно промоделируем работу алгоритмов компенсации при траекторных отклонениях на порядок больше размера ячейки РЛИ по дальности, т.е. *_dis_x_ampl = 4 м_*. Остальные параметры оставим без изменения. Сначала рассмотрим влияние искажений, затем их компенсация обоими методами.
