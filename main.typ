...
Рассмотрим детально процедуру компенсации для одной симуляции при _dis_x_ampl_ $ = 0,1$ м и _dis_x_freq_ $ = 2 slash T$ Гц (1 период). В соответствии с описанной ранне процедурой построим графики набега фазы в отклике цели, вызванного траекторными отклонениями (см. рис. @fig:prec_corr_phase_rcmc). В соответствии с процедурой алгоритма, описанной в гл. @ref:phase_multi, набег для центральной цели был скомпенсирован полностью, на графике набега фазы виден постоянный набег, и на полученном РЛИ (см. рис. @fig:prec_corr_all_targets) изображение данной цели полностью сфокусировано, о чём также говорят параметры в таблице @tab:prec_corr_target_params. Для остальных целей вид набега фазы включает остаточные искажения. Присутствующий шум на графиках может быть связан с некоторой неточностью определения сечения на голограмме. 
