...
#align(right, [
  * Автор: * \
  ...
  Гисич Арсений Витальевич \
  #v(2em)
  * Научный руководитель: * \
  Смирнов Василий Феликсович \
])
...
= Аннотация
Радиолокационное синтезирование апертуры (РСА) — технология дистанционного зондирования, обеспечивающая получение высокодетализированных изображений поверхности независимо от погодных условий. РСА использует активное облучение и приём отражённых сигналов, которые затем обрабатываются для получения радиолокационного изображения (РЛИ). Однако РСА имеет и ограничения: кратковременность наблюдения, возможные искажения из-за нестабильности движения носителя, уязвимость к помехам. Особенность авиационных РСА — высокая мобильность и оперативность, но при этом они подвержены траекторным нестабильностям (ТН), вызывающим расфокусировку. Современные системы навигации (ГНСС и инерциальные датчики) позволяют точно определять положение носителя и компенсировать ТН без процедуры автофокусировки. Целью работы является реализация такой компенсации для дальностно-доплеровского алгоритма и её верификация на модельных и реальных данных.

При реализации данной процедуры сформулированы и решены следующие задачи:

 - Теоретический анализ существующих подходов к компенсации траекторных нестабильностей;
 - Реализация модели отражённого сигнала РСА от точечных целей;
 - Реализация дальностно-доплеровский алгоритм синтеза РЛИ и процедуры компенсации траекторных отклонений;
 - Анализ качества реализованных алгоритмов коррекции траекторных искажений по модельным радиолокационным и навигационным данным;
 - По результатам анализа был сделан вывод о требованиях к точности навигационной информации для обеспечения заданных характеристик радиолокационных изображений (РЛИ);

 По результатам проведённого моделирования планируется отладка и применение реализованных алгоритмов для реальных данных с самолётного РСА макета РЛК Радиолокация-Х разработки АО РКС.
...
#let abbr = (
  ...
  (name: "РЛИ", desc: "радиолокационное изображение"),
  (name: "РЛС", desc: "радиолокационная станция"),
  (name: "РЛК", desc: "радиолокационный комплекс"),
  ...
  (name: "ПСФ",  desc: "принцип стационарной фазы"),
  (name: "ДДА",  desc: "дальностно-доплеровский алгоритм"),
)
...
= Введение
Радиолокационное синтезирование апертуры является одной из ведущих технологий дистанционного зондирования, позволяющей получать детальные изображения земной и водной поверхности вне зависимости от погодных условий @verba, @kondratenkov. Радиолокаторы с синтезированной апертурой (РСА) являются активными средствами наблюдения, т.е. используют облучение пространства зондирующим сигналом, приём и обработку отражённых сигналов. Принятый сигнал затем подвергается обработке для осуществления синтеза радиолокационных изображения, содержащих информацию о свойствах объекта наблюдения. Информационные продукты, создаваемые на основе этих данных, находят широкое применение в народном хозяйстве @kondor, @sentinel.

Основное отличие синтезированной апертуры от реальной заключается в том, что синтезированная апертура формируется последовательно во времени, при этом каждый момент времени приём сигнала производится реальной апертурой. Синтезирование апертуры осуществляется за счёт приёма сигнала реальной апертурой при различном её положении относительно источника. В каждом положении РЛС излучает зондирующей импульс и принимает его отражение от снимаемого участка земной поверхности --- сцены. Параметры принятого сигнала --- амплитуда и фаза записываются в запоминающее устройство и затем подвергается обработке различными алгоритмами для синтеза изображения. Такой метод позволяет получить апертуру, в несколько раз превышающую размер реальной апертуры. Благодаря б#math.accent("о", sym.acute)льшему размеру апертуры РСА возможно получение высокого линейного разрешения на больших дальностях, где оптические системы уже не позволяют наблюдать объекты. А последовательное излучение зондирующего сигнала в разных точках позволяет разделять сигнал от целей не только по дальности, но и по оси, параллельной движению носителя РСА --- оси азимута. 

Полученные отсчёты сигнала по оси и дальности после демодуляции формируют голограмму исходного сигнала. Для формирования РЛИ голограмма подвергается обработке различными алгоритмами синтеза @cumming_sar. Данные алгоритмы используют фазовую и амплитудную информацию об отражённых от целей и фона сигналах, содержащуюся в голограмме, а также информацию о геометрии съёмки и параметрах системы, которую затем преобразуют с помощью различных операций. Такие операции включают в себя методы согласованной фильтрации и интерполяции, и определяются исходя из математической модели отражённого сигнала. Различные алгоритмы синтеза имеют свои преимущества и недостатки в зависимости от геометрии съёмки и конфигурации системы. Одним из таких алгоритмов является дальностно-доплеровский алгоритм синтеза (см. гл. @ref:RDA). Полученное на выходе алгоритма синтеза РЛИ может быть частично дефокусировано вследствие различного рода искажений траекторного сигнала и неточностей алгоритма синтеза, связанных с использованием приближённых математических моделей. В таких случаях применяются дополнительные процедуры дофокусировки. 

Основным способом оценки качества синтеза изображения является оценка характеристик отклика точечных контрастных целей, в роли которых могут выступать миры, составленные из специально установленных на земле уголковых отражателей. На основании оценки параметров функции отклика такой цели, таких как ширина и мощность лепестков по осям азимута и дальности сцены, можно сделать вывод о качестве работы алгоритма синтеза. 

Таким образом, радиолокационная съёмка обладает рядом преимуществ в сравнении с оптическими средствами наблюдения @verba:
...
Вышеописанные особенности систем РСА позволяют использовать их в различных отраслях ДЗЗ. В настоящее время системы аэрокосмического мониторинга на базе РСА используются для решения следующих задач @neronsky: 
...
Одним из вариантов исполнения системы РСА является РСА авиационного базирования @airborne. Такой вариант обладает некоторыми важными преимуществами перед РСА космического базирования. В первую очередь, авиационные РСА обладают большей мобильностью, т.е. траектория носителя РСА может иметь практически любую геометрию, что позволяет использовать различные режимы работа в зависимости от задачи. Также немаловажно, что разработка и развёртывание таких систем может быть значительно проще и быстрее, что существенно повышает оперативность решения задач наблюдения. 

Однако, РСА авиационного базирования обладают существенным недостатком --- траектория движения носителя такой системы подвержена существенным искажениям относительно идеального равномерного и прямолинейного движения, вызванным флуктуациями и другими процессами, происходящими в атмосфере @radar_processing, @kondratenkov. Такие траекторные нестабильности (ТН) приводят к геометрическим искажениям и расфокусировке получаемых РЛИ. Вследствие этого, становится актуальной задача компенсации ТН при синтезе изображений. Традиционным способом решения данной проблемы является дополнительная процедура автофокусировки изображения (оценка оптимальных параметров синтеза) @kondratenkov, которая существенно увеличивает вычислительную нагрузку и приводит к возможной потере радиометрических и геометрических характеристик. Микроминиатюризация, усовершенствование инерциальных датчиков и высокоточных средств определения положения с помощью ГНСС (до нескольких сантиметров) позволяют в данный момент получать точную информацию о положении и ориентации носителя РСА во время съемки и учитывать ее для компенсации траекторных искажений @chen_sar, @moco_sar. Преимуществом данного подхода является возможность избежать процедуры автофокусировки путём корректировки существующих алгоритмов синтеза изображений с сохранением радиометрических и геометрических характеристик РЛИ. Таким образом, целью данной работы является реализация вышеописанной процедуры компенсации ТН для дальностно-доплеровского алгоритма, анализ качества реализации на модельных радиолокационных и навигационных данных и верификация с использованием реальных данных самолётного РСА. Обоснование выбора данного алгоритма и подходов к компенсации ТН приведено далее.

== Актуальность темы работы
Одной из проблем, возникающих при синтезе изображения, полученного с РСА, являются искажения траектории носителя РСА, которые могут приводить к существенным искажениям и расфокусировке полученных РЛИ. При компенсации данных искажений возникает необходимость точного знания фазы траекторного сигнала, что равносильно ошибке положения точек апертуры относительно заданного. Эта необходимость обусловлена тем, что существующие алгоритмы синтеза РЛИ, как правило, предполагают равномерное прямолинейное движение платформы РСА. Одной из основных причин возникновения этих ошибок являются траекторные нестабильности платформы РСА, которые особенно характерны для РСА авиационного базирования. Допустимая ошибка в знании траектории фазового центра антенны составляет несколько миллиметров для сантиметрового диапазона волн [кондратенков]. Этим обусловлена необходимость использования специальных алгоритмов компенсации траекторных нестабильностей.

* Целью работы * является исправление траекторных нестабильностей с помощью использования траекторной информации при синтезе радиолокационных изображений (РЛИ)

* Объект исследования *: траекторные нестабильности (ТН)

* Предмет исследования *: подходы к компенсации ТН

== Постановка задачи
Таким образом, при выполнении данной работы были сформулированы следующие задачи:

 - Проанализировать существующие подходы к компенсации траекторных нестабильностей;
 - Реализовать модель отражённого сигнала РСА от точечных целей;
 - Реализовать дальностно-доплеровский алгоритм синтеза РЛИ и процедуры компенсации траекторных отклонений;
...
= Схема работы РСА <ref:SAR_geometry>
Геометрия радиолокационного обзора представлена на рис. @fig:sar_geometry. По мере движения платформы, на которую установлена радиолокационная система, радар периодически излучает сигнал, после чего регистрирует его отражение от поверхности. ДН антенной системы направлена в сторону от траектории движения, в некоторых системах это направление не перпендикулярно ей, т.е. имеет место _угол скоса_.

Существуют различные режимы работы систем РСА, наиболее распространённые из них @verba:
...
- _обзорный режим_ --- маршрутный режим с увеличением ширины полосы съёмки путём последовательного переключения между несколькими лучами; при этом ухудшается разрешение по азимуту.

В данной работе рассматривается РСА в маршрутном режиме работы при нулевом угле скоса, так как такой режим работы является наиболее простым для моделирования и последующей оценки качества синтеза.

 #figure(
    image("images/SAR_geometry.svg", width: 70%),
    caption: [Геометрия работы РСА],
  )<fig:sar_geometry> 

Циклограмма сигнала, излучаемого и принимаемого антенной системой РСА, изображена на рис. @fig:recv_signal. В начале периода излучается сигнал длительностью $t_r$. Затем аппаратура переключается на приём. Принятый сигнал записывается с момента $t_1$ (соответствующего началу прихода отражённого сигнала от объектов на ближней границе $R_1$), до момента $t_2$ (соответствующего окончанию прихода отражённого сигнала от объектов на дальней границе $R_2$). Описанный цикл повторяется с интервалом _PRI_.

  #figure(
    image("images/recv_signal.svg", width: 70%),
    caption: [Циклограмма излучения и приёма сигнала РСА],
  )<fig:recv_signal>

#pagebreak()

= Алгоритмы синтеза РЛИ и коррекции ТН <ref:algorithms>

== Дальностно-доплеровский алгоритм <ref:RDA>
Дальностно-доплеровский алгоритм разработан в 1978 году и является одним из наиболее распространённых алгоритмов синтеза РЛИ @cumming_sar. Блок-схема алгоритма с модификациями для коррекции ТН (будут рассмотрены далее) представлена на рис. @fig:RDA_model. Отличительной особенностью алгоритма является операция коррекции миграции дальности, которая выполняется во временной области по дальности и в частотной области по азимуту (области доплеровских частот), т.е. в дальностно-доплеровской области. Математические преобразования, осуществляемые на каждом шаге, и их смысл рассмотрены далее.
...
Сигнал, отражённый от точечной цели после демодуляции, имеет вид:
...
$"где" quad &tau && dash.em quad "время по дальности ("quote.double"быстрое"quote.double")" \
&eta && dash.em quad "время по азимуту ("quote.double"медленное"quote.double")" \
&f_0 && dash.em quad "частота несущей" \
&K_r && dash.em quad "скорость ЛЧ-модуляции" \
&R(eta) quad && dash.em quad "наклонная дальность"$
...
где $R_0$ --- дальность ближайшего подлёта (см. рис. @fig:sar_geometry), а $eta = 0$ в момент ближайшего подлёта к данной цели (для нескольких целей $eta$ обозначает абсолютное время, и $eta = 0$ в центре изображения).

=== Сжатие сигнала по дальности
После операции сжатия по дальности континуальный по времени сигнал от цели, зашитый в голограмме, будет представлять из себя несколько отсчётов по данной оси, в которых будет сконцентрирована его мощность, что позволяет  разделить сигналы от разных источников. Данная операция производится путём умножения ПФ по дальности от исходного сигнала @eq:raw_signal на согласованный фильтр, представляющий из себя комплексно сопряжённое ПФ дополненного нулями единичного ЛЧМ-импульса длительностью $t_r$ (см. рис. @fig:recv_signal)
$ g(tau) = exp(j dot pi K_r tau^2), quad {-t_r slash 2 < tau < t_r slash 2}, $ после чего нужно взять ОПФ по дальности от полученного произведения. Полученное преобразование записывается как:
...
где $p_r (tau)$ --- функция $~sinc$.

=== БПФ по азимуту
БПФ по азимуту переводит голограмму в частотную (доплеровскую) область по азимуту, благодаря чему отклики стоящих на одной дальности целей будут располагаться на одной линии (для них будет одинаковый закон изменения дальности в новых координатах дальность-доплеровская частота). Для малых углов скоса можно воспользоваться приближением @eq:slant_range:
...
Выполняется путём умножения на согласованный фильтр в частотно-доплеровской области для того, чтобы уменьшить искажения, связанные с взаимозависимостью дальности и азимута друг от друга. В вышеописанных формулах эта зависимость опущена для упрощения, вывод же более точных соотношений весьма громоздкий и здесь не приводится. В случае малых углов скоса данной операцией можно принеберечь.

=== Коррекция миграции дальности
Коррекция миграции дальности устраняет зависимость наклонной дальности до целей от движения носителя, после чего соответствующие одной дальности отклики будут располагаются на одной прямой, параллельной оси азимута. Эта операция производится путём интерполяции в частотной по дальности и азимуту области. Из @eq:slant_range_approx_f_eta получаем $ R_(r d) (f_eta) = R_0 (1 + (lambda^2 f_eta^2) / (8 v_r^2)), $ т.е. миграция дальности возникает за счёт умножения дальности ближайшего подлёта на зависящий от $f_eta$ коэффициент. Интерполяция на новую дальность сигнала после предыдущего шага позволяет избавиться от этого множителя, в итоге получаем
$ S_2 (tau, f_eta) &= \
&= A_0 dot p_r (tau - (2R_0) / c) dot W_a (f_eta - f_eta_c) dot exp(-j (4pi f_0 R_0) / c) dot exp(j pi f_eta^2 / K_a) $<eq:after_rcmc>

=== Сжатие по азимуту
Данная операция выполняется путём умножения на согласованный фильтр такой, чтобы исключить квадратичную по $f_eta$ компоненту фазы и сфокусировать отклик в азимутальном направлении. Такой фильтр имеет вид 
...
=== ОБПФ по азимуту
Финальная операция завершает сжатие по азимуту, после чего мы получаем сфокусированное изображение цели, сигнал отклика которой имеет вид
...
== Компенсация траекторных нестабильностей <ref:MOCO>
Для РСА авиационного базирования траекторные искажения вносят существенную фазовую ошибку (в сравнении с длиной волны в единицы см @verba), вызывая расфокусировку РЛИ. Следствиями фазовых искажений при синтезе РЛИ являются: 
- Смещение максимума отклика цели, что приводит к ошибкам измерения координат объектов
- Уменьшение максимума выходного сигнала, что равносильно снижению энергетического потенциала РЛС
- Расширения главного лепестка отклика цели, что приводит к ухудшению разрешающей способности
...
* Автофокусировка по сдвигу изображения (Map-drift) @verba. *
Структурная схема алгоритма представлена на рис. @fig:Map-Drift_model. Исходную радиоголограмму делят на две неперекрывающиеся подапертуры, по каждой из которых производят синтез РЛИ с использованием опорной функции, рассчитанной на основе баллистических параметров. После детектирования каждого из амплитудных РЛИ --- вычисляют их корреляционную функцию. Для устранения ошибок более высокого порядка, чем второй, голограмму делят на несколько пар парциальных апертур с совместной обработкой с совместной обработкой каждой пары амплитудных РЛИ для расчёта коэффициентов разложения в ряд.

  #figure(
    image("images/Map-Drift.svg", width: 100%),
    caption: [Блок-схема автофокусировки по сдвигу изображения],
  )<fig:Map-Drift_model>

=== Фазовые множители <ref:phase_multi>
Данный подход основан на использовании навигационных данных, полученных от БИНС платформы. БИНС самолёта или БПЛА позволяет с помощью данных ГНСС или инерциальных измерений получить фактическое положение платформы в момент приёма отражённого сигнала. Таким образом, можно вычислить расстояние между фактическим и предполагаемым положением фазового центра антенны в момент приёма. Эта разница равносильна фазовой ошибке и дополнительной задержке сигнала, которая искажает отклик цели и должна быть исправлена. В данной работе коррекция траекторных искажений выполняется двумя методами (см. рис. @fig:RDA_model): «базовым» @zaugg_theory и «приближённым» @zaugg_generalized.
...
В результате влияния траекторных нестабильностей изменяется дальность до цели (см. рис. @fig:MOCO_geometry), тем самым изменяется фаза отклика цели и его момент регистрации (см. рис. @fig:recv_signal_error), то есть отклику данной цели будет соответствовать иное положение на оси времени и, соответственно, дальности. В данном методе изменение фазы компенсируется умножением на согласованный фильтр, зависящим от времени по азимуту и от искажения траекторного сигнала на заданной дальности
...
= Реализация модели 
== Реализация модели для точечных сигналов <ref:model>
Для анализа и оценки рассматриваемых подходов к компенсации ТН используется модель РСА авиационного базирования, блок-схема которой представлена на рис. @fig:model_scheme.
...
Входные параметры модели описаны в таб. @tab:model_params.
...
#figure(
  table(
    ...
    [$"res"_a$], [Разрешение по азимуту],
    [$"res"_r$], [Разрешение по дальности],
    [$h$], [Высота полёта],
    [$alpha$], [Угол раствора ДН],
    [$v$], [Скорость движения платформы],
    [$t_r$], [Длительность импульса],
    [_bw_], [Ширина полосы излучаемого ЛЧМ сигнала],
    [_prf_], [Частота повторения импульса],
    [$f_0$], [Частота несущей],
    [$y_0$], [Начальная координата платформы по оси _Y_],
    [$R_"min"$], [Ближняя граница],
    [$R_"max"$], [Дальняя граница],
    [$T$], [Время пролёта],
    [_dis_x_ampl_], [Амплитуда ТН по оси _X_],
    [_dis_x_freq_], [Частота ТН по оси _X_],
    [-], [Координаты целей],
  ),
  caption : [Входные параметры модели для точечных целей]
)<tab:model_params>

В модели предполагается (см. рис. @fig:model_geometry), что платформа (в идеальном случае) движется в 3-х мерном декартовом пространстве вдоль оси _X_ (т.е. по прямой $x = 0$) начиная с координаты $y_0$ в течение времени $T$ с постоянной скоростью $v$ на постоянной высоте $h$ по оси _Z_. Точечные цели расположены на высоте $h = 0$ в указанных координатах $(x, y)$. Диаграмма направленности имеет вид функции $sinc^2("scale" dot x)$ такой, что координаты $x = plus.minus alpha$ соответствуют уменьшению функции на 3 дБ относительно максимума, а максимум функции соответствует минимальной дальности подлёта к цели. Визуализация параметров зондирующего импульса представлена на рис. @fig:recv_signal.
...
На данном этапе производится интерполяция изображения каждой точечной цели, которое затем используется для оценки полученного разрешения и иных параметров. Алгоритм анализа подробно описан в следующем пункте.

== Анализ функции отклика точечной цели на РЛИ
Оценка качества полученных изображений производится по следующим параметрам: разрешение по азимуту и дальности, относительный и интегральный уровень мощности боковых лепестков. Алгоритм анализа отклика цели представлен на рис. @fig:image_analysis_model.
...
Значения параметров модели, использованные при симуляциях, приведены в таб. @tab:sim_params. 
#figure(
  table(
    columns: (auto, auto),
    align: (right, left),
...
Координата $y_0$ выбрана для простоты таким образом, чтобы цели на оси $x = 0$ были на середине изображения. Координаты целей приведены в таб. @tab:target_coords. 
#figure(
  table(
    columns: (auto, auto),
    align: (right, left),
...
Амплитуда ТН _dis_x_ampl_ изменяется от 0 до 0,5 м с шагом 0,1 м. Значения частоты функции ТН _dis_x_freq_ взяты так, чтобы на длине пути укладывались соответственно 1, 2 и 3 её периода. Диапазоны значений выбраны так, чтобы наглядно пронаблюдать изменение изображения от сфокусированного до его разрушения. Исходя из описания алгоритмов компенсации и общих принципов ЦОС @signals, предполагается, что при увеличении амплитуды ТН измеренное разрешение по азимуту будет экспоненциально ухудшаться. А при увеличении частоты ТН оно будет несущественно улучшаться, так как при увеличении частоты функция ТН будет лишь ухудшать соотношение сигнал-шум и тем самым увеличивать уровни боковых лепестков, не влияя на разрешение.

== Проверка реализации ДДА <ref:no_dis>
Для проверки корректности работы реализации алгоритма сначала проводится симуляция без добавления ТН. Значения параметров симуляции приведены в таб. @tab:rda_model_params. Координаты целей приведены в таб. @tab:target_coords.
...
Также на рис. @fig:no_dis_scene -- @fig:no_dis_targets представлено изображение сцены и РЛИ всех целей.
...
Рассмотрим эффекты, возникающие на промежуточных этапах обработки голограммы и на конечном изображений вследствие наличия ТН. Значения параметров модели приведены в таб. @tab:no_corr_model_params. Координаты целей аналогичны предыдущему пункту @ref:no_dis (см. таб.у @tab:target_coords). Значение амплитуды искажения траектории выбрано из начала диапазона, чтобы продемонстрировать начало разрушения изображения с не слишком большими искажениями.
...
Рассмотрим подробнее голограмму после коррекции миграции дальности. На рис. @fig:no_corr_phase_rcmc, а) можно заметить, что набег фазы соответствует искажению траектории, показанном на рис. @fig:no_corr_dR. К изображению @fig:no_corr_phase_rcmc, б) применён полосовой фильтр в диапазоне реальных доплеровских частот для центра изображения (элементы массива голограммы по азимуту соответствуют полосе частот _[-prf, +prf]_). Данное действие является частью алгоритма синтеза и используется для улучшения характеристик получаемых РЛИ. Также стоит отметить, что операция коррекция миграции дальности позволяет исправить искривления отклика цели вдоль оси азимута (в сравнении с искажениями на рис. @fig:no_corr_dR), но не набег фазы, который и вносит основной вклад в размытия РЛИ. Данный набег и предстоит убрать для исправления размытия РЛИ, т.е. должен остаться только линейный набег фазы. Далее в работе вид графика фазы на данном шаге будет использоваться для оценки качества работы процедур компенсации.
...
Приведём также полученные изображения всех целей (см. рис. @fig:no_corr_all_targets). Отчётливо видно, что даже искажение менее величины разрешения по дальности уже приводит к существенной расфокусировки РЛИ. Определение параметров функций отклика в данном случае не имеет смысла. 
...
Рассмотрим детально процедуру компенсации для одной симуляции при _dis_x_ampl_ $ = 0,1$ м и _dis_x_freq_ $ = 2 slash T$ Гц (1 период). В соответствии с описанной ранне процедурой построим графики набега фазы в отклике цели, вызванного траекторными отклонениями (см. рис. @fig:prec_corr_phase_rcmc). В соответствии с процедурой алгоритма, описанной в гл. @ref:phase_multi, набег для центральной цели был скомпенсирован полностью, на графике набега фазы виден постоянный набег, и на полученном РЛИ (см. рис. @fig:prec_corr_all_targets) изображение данной цели полностью сфокусировано, о чём также говорят параметры в таб. @tab:prec_corr_target_params. Для остальных целей вид набега фазы включает остаточные искажения. Присутствующий шум на графиках может быть связан с некоторой неточностью определения сечения на голограмме. 
...
Полученные РЛИ целей представлены на рис. @fig:prec_corr_all_targets. В данной симуляции для использованной величины траекторных отклонений видимые искажения на РЛИ отсутствуют, все цели сфокусированы.
...
Теперь проведём симуляции во для всех значений параметров ТН и построим соответствующие зависимости измеренных параметров функций отклика целей от амплитуды и частоты ТН, которые изображены на рис. @fig:prec_corr_res_graph, @fig:prec_corr_pslr_graph, @fig:prec_corr_islr_graph. Каждая точка на графике отображает усреднённое по всем целям разрешение. Как и было предсказано, на графики видно экспоненциальное ухудшение разрешения при увеличении амплитуды ТН.
...
#let approx_corr_res = csv("images/approx_sim/sim2_T1/results.csv")
 ...
 Теперь проведём симуляции во для всех значений параметров ТН и построим соответствующие зависимости измеренных параметров функций отклика целей от амплитуды и частоты ТН, которые изображены на рис. @fig:approx_corr_res_graph, @fig:approx_corr_pslr_graph, @fig:approx_corr_islr_graph. Каждая точка на графике отображает усреднённое по всем целям разрешение. Как и было предсказано, на графики видно экспоненциальное ухудшение разрешения при увеличении амплитуды ТН.
...
Аналогично предыдущим пунктам, рассмотрим соответствующие этапы синтеза РЛИ. На рис. @fig:no_corr_dR_big приведены графики искажения наклонных дальностей, результат которых модно наблюдать на этапах синтеза из рис. @fig:no_corr_fragments_big и полученных РЛИ целей на рис. @fig:no_corr_all_targets_big в виде полного разрушения изображений. На фрагменте рис. @fig:no_corr_phase_rcmc_big для цели #2 обратим внимание, что её отклик уже не близок к прямой линии, а оказывается существенно искривлён.
...
=== Компенсация "базовым" методом
Применим "базовый" метод для компенсации искажений. В соответствии с процедурой алгоритма, у всех целей практически отсутствует фазовый набег (см. рис. @fig:PrCorr_phase_rcmc_big). Фазовые искажения всех целей в целом исправлены, и цели изображения целей будут сфокусированы (см. рис. @fig:PrCorr_all_targets_big). Отметим также, что благодаря компенсации ошибки времени регистрации отклика цели, амплитуда откликов (см. рис. @fig:PrCorr_target_rcmc_big) практически полностью выпрямляется, что вносит вклад в компенсацию отклонений.
...
В отличие от предыдущего пункта, "приближённому" методу не удаётся справиться с ТН. Существенный статочный набег фазы (см. рис. @fig:ApCorr_phase_rcmc_big) и искривление амплитуды отклика (см. рис. @fig:ApCorr_target_rcmc_big), вызванное существенно большей ошибкой времени регистрации приводят к тому, что РЛИ всех целей оказываются разрушены, что показано на рис. @fig:ApCorr_all_targets_big.
...
= Заключение
В данной работе представлен теоретический анализ подходов к компенсации траекторных отклонений, возникающих при движении носителя РСА авиационного базирования, реализация выбранных процедур компенсации и анализ качества их работы. 

Для проведения теоретического анализа процедур компенсации в главе 1 была представлена схема работы РСА. Существенной проблемой, возникающей при съёмке с РСА авиационного базирования, являются нестабильности траектории, приводящие к расфокусировке получаемых изображений, что делает актуальной задачу их компенсации В разделе @ref:RDA приведён анализ алгоритмов синтеза РЛИ. Для синтеза изображений выбран дальностно-доплеровский алгоритм как наиболее простой в реализации, а также позволяющий проводить контроль промежуточных этапов синтеза на каждом шаге. В разделе @ref:MOCO представлен анализ существующих подходов к компенсации ТН. Данные подходы можно разделить на два класса: алгоритмы автофокусировки и алгоритмы, основанные на применении фазовых множителей. Первый подход заключается в том, что для высокоточного измерения фазовых искажений используется информация о фазовых нестабильностях, содержащейся в самой голограмме или РЛИ. Альтернативой являются методы, использующие навигационные данные, полученные от БИНС носителя для вычисления искажения траектории, и затем для их компенсации. С развитием навигационных систем появилась возможность с достаточной точностью определять положение и ориентацию носителя РСА во время съёмки, что делает данное направление перспективным для изучения. Целью работы является реализация такой компенсации для дальностно-доплеровского алгоритма и её верификация на модельных и реальных данных.

В работе для оценки качества работы алгоритмов компенсации в среде Matlab была разработана модель сигнала РСА авиационного базирования, позволяющая задавать геометрию сцены и траекторию носителя. Моделируемыми объектами являются точечные отражатели. Искажение траектории представляется в виде гармонической функции, прибавляемой к одной из осей координат носителя. Компенсация траекторных отклонений осуществляется двумя методами, основанными на исправлении фазы откликов цели: «базовым» и «приближённым»; их описание приведено в соответствующей главе. Оценка качества полученных изображений выполняется на основе анализа функции отклика точечных целей по следующим параметрам: разрешение по азимуту и дальности, относительный и интегральный уровень мощности боковых лепестков.

Для моделирование работы алгоритмов компенсации симуляции производится в следующем порядке: сначала проверяется реализации ДДА; затем демонстрируется влияние траекторных искажений на изображение; после чего проводится исследование алгоритмов компенсации при различных параметрах ТН. На каждом этапе проводится анализ промежуточных и итоговых результатов работы модели для оценки соответствующих параметров. При исследовании алгоритмов компенсации определяется изменение измеренного разрешения по азимуту (как основной параметр функции отклика) при увеличении амплитуды траекторных отклонений. Данная зависимость исследуется при изменении других параметров ТН, таких как период или вид функции отклонения. На основании данных измерений делается вывод о допустимой величине ТН для каждого алгоритма и пределах применимости данных алгоритмов.

Таким образом, в результате выполнения работы решены следующие задачи:

 - Проведён теоретический анализ существующих подходов к компенсации траекторных нестабильностей;
 - Реализована модель отражённого сигнала РСА от точечных целей;
 - Реализован дальностно-доплеровский алгоритм синтеза РЛИ и процедуры компенсации траекторных отклонений;
 - Проведён анализ качества реализованных алгоритмов коррекции траекторных искажений по модельным радиолокационным и навигационным данным;
 - По результатам анализа был сделан вывод о требованиях к точности навигационной информации для обеспечения заданных характеристик радиолокационных изображений (РЛИ);

 В результате выполненных исследований поставленная цель достигнута, по результатам оценки параметров функции отклика точечных целей после применения процедур компенсации было восстановлено требуемое качество РЛИ. По результатам оценки применимости методов приближённый метод даёт существенно более плохое качество изображения, следовательно базовый метод является более предпочтительным к использованию.

 Дальнейшими направлениями исследования может быть моделирование компенсации ТН при ненулевых углах скоса и различных видах функций искажения траектории, которые не были рассмотрены в данной работе. Также реализованные алгоритмы могут быть отлажены и применены для реальных данных с самолётного РСА макета РЛК Радиолокация-Х разработки АО РКС.
